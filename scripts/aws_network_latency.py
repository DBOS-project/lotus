...
for protocol in protocols: 
  for delay in delays:
    for i in range(3):
      cmd = get_cmd(n, i)
      print('./bench_tpcc --logtostderr=1 --id=%d --servers="%s" --protocol=%s --partition_num=%d --threads=12 --partitioner=hash2 --group_time=100 --delay=%d --query=mixed --neworder_dist=10 --payment_dist=15' % (id, cmd, protocol, 12*n, delay))   
