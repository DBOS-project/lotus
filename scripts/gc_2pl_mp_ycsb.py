...
for i,ratio in enumerate(ratios):
  cmd = get_cmd(n, i)
  print('./bench_ycsb --logtostderr=1 --id=%d --servers="%s" --threads=6 --read_write_ratio=50 --partition_num=18 --keys=100000 --granule_count=1 --log_path=/mnt/disks/nvme/coord0 --persist_latency=0 --wal_group_commit_time=1000 --wal_group_commit_size=3 --partitioner=hash2 --hstore_command_logging=false --protocol=TwoPL --replica_group=3 --lock_manager=1 --batch_flush=1 --cross_ratio=%d --lotus_async_repl=false --cross_part_num=5' % (id, cmd, ratio) )
