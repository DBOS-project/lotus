...
for ratio in ratios:
  for i in range(3):
    cmd = get_cmd(n, i)
    print('./bench_ycsb --logtostderr=1 --id=%d --servers="%s" --protocol=SiloGC --partition_num=%d --threads=12 --partitioner=pb --read_write_ratio=90 --cross_ratio=%d --batch_flush=200' % (id, cmd, 48, ratio))
      
for ratio in ratios:
  for i in range(3):
    cmd = get_cmd(n, i)
    print('./bench_tpcc --logtostderr=1 --id=%d --servers="%s" --protocol=SiloGC --partition_num=%d --threads=12 --partitioner=pb --query=mixed --neworder_dist=%d --payment_dist=%d' % (id, cmd, 48, ratio, ratio))  
